...
class Database(jdbcUrl:String,username:String,password:String){
    ...
    suspend fun <T> query(block:(Connection)->T):T=withContext(Dispatchers.IO){dataSource.connection.use(block)}
    fun migrate(){
        dataSource.connection.use{c->
            c.createStatement().use{statement->
                SCHEMA.split(";").map(String::trim).filter(String::isNotEmpty).forEach{sql->statement.execute(sql)}
            }
        }
    }
    fun close()=dataSource.close()
    companion object{
        private val SCHEMA="""CREATE TABLE IF NOT EXISTS users (uid TEXT PRIMARY KEY,email TEXT,nickname TEXT NOT NULL,avatar_url TEXT,bio TEXT,last_seen_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS watch_room_members (room_id UUID NOT NULL REFERENCES watch_rooms(id) ON DELETE CASCADE,uid TEXT NOT NULL REFERENCES users(uid) ON DELETE CASCADE,joined_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),PRIMARY KEY(room_id,uid));""".trimIndent()
    }
}
