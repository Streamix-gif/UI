...
class MediaAdaptor(
            ...
            0 -> {
                ...
                if (media != null) {
                    if (media.id < 0) {
                        ...
                        b.itemCompactTitle.background = ContextCompat.getDrawable(
                            b.root.context,
                            R.drawable.skeleton_text_placeholder
                        )
    ...
    inner class MediaPageSmallViewHolder(val binding: ItemMediaPageSmallBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
            binding.itemCompactImage.setSafeOnClickListener { clicked(bindingAdapterPosition, binding.itemCompactImage) }
            binding.itemCompactTitleContainer.setSafeOnClickListener { clicked(bindingAdapterPosition, binding.itemCompactImage) }
            itemView.setOnTouchListener { _, _ -> true }
            binding.itemCompactImage.setOnLongClickListener { longClicked(bindingAdapterPosition) }
        }
    }

    fun clicked(position: Int, itemCompactImage: android.widget.ImageView? = null) {
