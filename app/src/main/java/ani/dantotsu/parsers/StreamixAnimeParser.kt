...
class StreamixAnimeParser : AnimeParser() {
    ...
    override suspend fun loadVideoServers(
        episodeLink: String,
        extra: Map<String, String>?,
        sEpisode: eu.kanade.tachiyomi.animesource.model.SEpisode
    ): List<VideoServer> {
        val episode = decodeEpisode(episodeLink)
        return service.providerStreams(episode).mapIndexed { index, stream ->
            VideoServer(streamName(stream, index), encodeStream(stream))
        }
    }

    override suspend fun getVideoExtractor(server: VideoServer): VideoExtractor? =
        StreamixVideoExtractor(server, decodeStream(server.embed.url))

    private fun ProviderAnime.toShowResponse() =
        ShowResponse(
            name = title,
            link = encodeAnime(providerId, id),
            coverUrl = defaultImage,
            sAnime = eu.kanade.tachiyomi.animesource.model.SAnime.create().apply {
    ...
    private fun EpisodeRef.toEpisode() =
        Episode(
            number = number.toString(),
            link = encodeEpisode(providerId, providerEpisodeId, url),
            title = title,
            sEpisode = eu.kanade.tachiyomi.animesource.model.SEpisode.create().apply {
    ...
    private fun encodeAnime(providerId: String, id: String) =
        pack("A|$providerId|$id")

    private fun encodeEpisode(providerId: String, episodeId: String, url: String) =
        pack("E|$providerId|$episodeId|$url")

    private fun encodeStream(stream: StreamRef) =
        pack("S|${stream.providerId}|${stream.url}|${stream.quality ?: -1}|${stream.type ?: ""}")

    private fun pack(value: String) =
        Base64.encodeToString(value.toByteArray(Charsets.UTF_8), Base64.NO_WRAP)

    private fun unpack(value: String) =
    ...
    private fun decodeStream(value: String): StreamRef {
        ...
        return StreamRef(
            providerId = parts[1],
            url = parts[2],
            quality = parts[3].toIntOrNull()?.takeUnless { it < 0 },
            type = parts[4].ifBlank { null }
        )
    }

    private fun streamName(stream: StreamRef, index: Int) =
        stream.quality?.takeIf { it > 0 }?.let { "${it}p" } ?: "Stream ${index + 1}"
}
...
private object StreamixProviderBridge {
    ...
    fun service(context: Context): StreamixService =
