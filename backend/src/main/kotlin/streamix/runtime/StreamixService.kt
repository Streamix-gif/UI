...
class StreamixService(
    ...
    /** Direct provider bridge for app-side source adapters. */
    suspend fun providerEpisodes(providerId: String, providerAnimeId: String): List<EpisodeRef> =
        router.provider(providerId)?.loadEpisodes(providerAnimeId).orEmpty()

    /** Direct provider bridge for app-side source adapters. */
    suspend fun providerStreams(episode: EpisodeRef): List<StreamRef> =
        router.streams(episode)
}
