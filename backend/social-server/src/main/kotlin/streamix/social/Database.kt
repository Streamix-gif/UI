...
class Database(jdbcUrl:String,user:String,password:String){
    private val pool=HikariDataSource(HikariConfig().apply{
        this.jdbcUrl=jdbcUrl;username=user;this.password=password;maximumPoolSize=10;minimumIdle=1
    })
    suspend fun <T> query(block:(Connection)->T):T=withContext(Dispatchers.IO){pool.connection.use(block)}
    fun migrate(){pool.connection.use{c->c.createStatement().use{st->SCHEMA.split(";").map(String::trim).filter(String::isNotEmpty).forEach(st::execute)}}}
